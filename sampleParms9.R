sampleParms9 <-
function(X, GLOBvar, HYPERvar, s_init=NULL, options, hard.const){
  ...
  for(target in 1:length(X)) {
    ...
    for (i in 1:(s[target]+1)){
      ...
      # NEW to v9:
      # If a hard constraint is imposed by the user, the network for the final segment is initialized with the constrained edges. Otherwise, the original random initialization is applied.
      
	  if ((i>s[target]) && (length(intersect(target,hard.const[,2]))>0))
	  {
	  	hard.par = as.numeric(hard.const[hard.const[,2]==target,1])
	  	S[[target]][i,hard.par] = array(1,length(hard.par))
	  	if (kPred > length(hard.par))
	  	{
	  		non.par = c(1:q)[!(c(1:q) %in% hard.par)]
	  		kPred.diff = kPred-length(hard.par)
	  		S[[target]][i, sample(non.par, kPred.diff, replace=FALSE)] = array(1, kPred.diff) 
	  	} 
	  } else {
      	if(kPred>0){
