...
function(Eall, Sall, Ball, Sig2all, X, Y, D, GLOBvar, HYPERvar,
  ...
  # NEW to v9:
  # if target has a hard constraint and last CP is being removed, choose to extend the original network 
  # model for the last time segment
  if ((length(intersect(target,hard.const[,2]))>0) && (estar==E[length(E)-1]))
  	newRight = 1 else newRight = sample(0:1,1)
