...
function(Eall, Sall, Ball, Sig2all, X, Y, D, 
                     GLOBvar, HYPERvar, target, hard.const){
  ...
  # Sample the new CP "estar"
  	...
  	# NEW to v9:
  	# skip this CP birth if there are no possible places to insert CP

  if (length(possibleCP)<1)
  {
   	accept=0
  	alpha=0
  	estar=0
  ...
  # NEW to v9:
  # if target has a hard constraint on its parents and the new CP is later than existing CPs, then 
  # keep the original network as the model for the newRight segment

  if ((length(intersect(target,hard.const[,2]))>0) && (estar>E[length(E)-1]))
  	newRight = 0 else newRight = runif(1,0,1) >= 1/2
